...
def transferir(request):
    ...
    if destino and monto <= saldo:
        ...
        _log_movimiento_(id_usuario_destino, id_destino,
                         id_origen, descripcion, monto)
...
def _log_movimiento_(id_usuario, cuenta_origen, cuenta_destino, descripcion, monto):
    ...
    db = Database()
    db.logear_movimiento(id_usuario, cuenta_origen,
                         cuenta_destino, descripcion, monto, fecha)
...
def escanear(request):
    ...
    cv2.imwrite("nombre_del_proyecto/static/img/qr.jpg", image)
    print(result)
    try:
        for _barcode in decode(image):
            barcode = _barcode
        destino = barcode.data.decode('utf-8')
        operacion = "qr leido con exito"
    except Exception as e:
        print(e.with_traceback)
        operacion = "No se pudo leer el qr"
        destino = None
        return render(request, "scan.html", {"id": id, "operacion": operacion})

    id_usuario = request.session["id"]
    db = Database()
    cuentas = db.get_cuentas(id_usuario)
    os.remove('nombre_del_proyecto/static/img/qr.jpg')
    return render(request, "cuentas.html", {"operacion": operacion, "cuentas": cuentas, "destino": destino})